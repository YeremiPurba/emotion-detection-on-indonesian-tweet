...
const DUMMY_LABELS = ['Anger', 'Happy', 'Sadness', 'Fear', 'Love']
const DUMMY_SCORES = [73, 81, 87, 79, 92]
...
export default function Beranda() {
  ...
  const [result, setResult] = useState(null) // { label, score, tweetText }
  ...
  const handleDeteksi = async () => {
    ...
    setLoading(true)
    // TODO: ganti dengan axios.post('/predict', { text: teks })
    await new Promise((r) => setTimeout(r, 500)) // simulasi delay request
    const label = DUMMY_LABELS[Math.floor(Math.random() * DUMMY_LABELS.length)]
    const score = DUMMY_SCORES[Math.floor(Math.random() * DUMMY_SCORES.length)]

    setResult({ label, score, tweetText: teks })
  ...
  const handleChooseFeedback = (choice) => {
    ...
    if (choice === 'benar') setKoreksiLabel(null)
  }
  ...
  const handleSaveFeedback = () => {
    ...
    // TODO: ganti dengan axios.post('/feedback', { deteksi_id, status: feedbackChoice, label_koreksi_id })
    setSaved(true)
  }
  ...
  const currentStep = !result ? 1 : saved ? 4 : feedbackChoice ? 3 : 2

  return (
    <div className="min-h-screen bg-bg font-sans">
      <Navbar showLinks activePage="beranda" userName={user?.nama_lengkap ?? 'Pengguna'} />
      ...
      <div className="mx-auto max-w-180 px-6 pb-16 pt-8">
        ...
        {!result ? (
          <TweetInputForm value={teks} onChange={setTeks} onSubmit={handleDeteksi} loading={loading} />
        ) : (
          <div className="mb-4 flex items-center justify-between rounded-2xl border border-border bg-white p-4">
            <div className="min-w-0 flex-1">
              <div className="mb-0.5 text-xs text-text-muted">Teks yang dianalisis</div>
              <div className="truncate text-[13.5px] text-text-primary">{result.tweetText}</div>
            </div>
            <button
              onClick={handleReset}
        ...
        {result && (
          <>
            <PredictionResult label={result.label} score={result.score} tweetText={result.tweetText} />
            <FeedbackForm
